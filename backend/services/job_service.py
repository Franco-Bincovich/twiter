...
Orquesta validación, creación y procesamiento. El enriquecimiento real (BCRA núcleo + ARCA
y BORA complementarios, con caché e informes IA) vive en `pipeline_service.ejecutar_pipeline`;
acá solo se gestiona el ciclo de vida del job (PENDING → PROCESSING → DONE/ERROR).
"""
...
async def procesar_job(job_id: UUID) -> Job:
    ...
    Delega el enriquecimiento en `pipeline_service.ejecutar_pipeline`. Un `AppError` del
    pipeline (núcleo BCRA) marca el job en ERROR con su `code`; cualquier otra excepción se
    reporta como 'PIPELINE_ERROR'. ARCA/BORA degradan dentro del pipeline y no tumban el job.
    ...
    try:
        resultado = await pipeline_service.ejecutar_pipeline(job)
